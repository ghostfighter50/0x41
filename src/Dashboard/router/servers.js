...
router
	...
	.post("/:guildID/settings", CheckAuth, async function (req, res) {
		...
		if (req.body.Raidmode !== "none" || req.body.RaidMode.length > 0) {
			manager.RaidMode(req.body.RaidMode);
		}
		if (req.body.Levels !== "none" || req.body.Levels.length > 0) {
			manager.Levels(req.body.Levels);
		}
		if (
			req.body.SkidVerification !== "none" ||
			req.body.SkidVerification.length > 0
		) {
			manager.SkidTest(req.body.SkidVerification);
		}
		if (req.body.JoinLogger !== "none" || req.body.JoinLogger.length > 0) {
