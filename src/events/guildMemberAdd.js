module.exports = async (client, member) => {
  ...
  if (client.serverconfig[member.guild.id].RaidMode == true) { return await member.kick() }
  ...
  client.serverconfig[member.guild.id].autoroles.forEach((role) => {
    const autorole = member.guild.roles.cache.find((r) => r.id == role)
  ...
  const delrole = member.guild.roles.cache.find(
  ...
  if (client.serverconfig[member.guild.id].SkidVerification == false) return
    ...
    const question1 = await member.send(
    ...
    question1.channel
      ...
      .then(async (collected) => {
        if (
          ...
          const question2 = await member.send(
            new Discord.MessageEmbed()
              .setDescription(
          ...
          question2
            ...
            .then(async (collected) => {
              if (collected.first().emoji.name == '2️⃣') {
                ...
                const question3 = await member.send(
                  new Discord.MessageEmbed()
                    .setDescription(
