exports.run = async (client, message) => {
	...
	let verified = message.guild.roles.cache.find(
		(r) => r.id == client.config[message.guild.id].VerifiedRole
	);
	let unverified = message.guild.roles.cache.find(
		(r) => r.id == client.config[message.guild.id].UnverifiedRole
	);
	let report = message.guild.channels.cache.find(
		(r) => r.id == client.config[message.guild.id].ReportChannel
	);
	let test = message.guild.channels.cache.find(
		(r) => r.id == client.config[message.guild.id].TestChannel
	);
	let welcome = message.guild.channels.cache.find(
		(r) => r.id == client.config[message.guild.id].WelcomeChannel
	);
	...
	const embed = new Discord.MessageEmbed()
		...
		.addField(
			...
			client.config[message.guild.id].SkidVerification,
			true
		)
		.addField(
			"**Join Logger **",
			client.config[message.guild.id].JoinLogger,
			true
		)
		.addField("**Raid Mode **", client.config[message.guild.id].RaidMode, true)
		...
		.addField(
			"**Join Message **",
			client.config[message.guild.id].JoinMessage,
			true
		)
		.addField(
			"**Leave Message **",
			client.config[message.guild.id].LeaveMessage,
			true
		)
		.addField(
			"**Embed color **",
			client.config[message.guild.id].EmbedColor,
			true
		)
		.setTitle(`📖 Config `)
